...
if matching_files:
    with xr.open_mfdataset(r'/projects/0/einf2224/paper2/scripts/model_runs/impact/submodel_' + str(submodel_id) + '_damage_build_*.nc', chunks='auto', drop_variables='depth') as submodel_dbuild_whole_orig:
        ...
        #for i, row in list(region_gdf.iterrows())[6:7]:
            ...
            try:
              submodel_dbuild_whole=submodel_dbuild_whole_orig

              
              # drop events that activated all submodels
              submodel_dbuild_whole = drop_events(submodel_dbuild_whole) 
              
              if not submodel_dbuild_whole.data_vars:
                  ...
                  # convert to integers
                  submodel_dbuild_whole = convert_to_integers(submodel_dbuild_whole)
                  #print('submodel_dbuild_whole:', submodel_dbuild_whole)
                  submodel_dbuild_whole=submodel_dbuild_whole.squeeze(drop=True)
                  #print('submodel_dbuild_whole:', submodel_dbuild_whole)
                  #submodel_dbuild_whole=submodel_dbuild_whole['stormid_637805']
                  variable_names = list(submodel_dbuild_whole.data_vars.keys())
                  selected_variables = variable_names[int(var1):int(var2)]
                  submodel_dbuild_whole = submodel_dbuild_whole[selected_variables]
                  submodel_dbuild=submodel_dbuild_whole.raster.clip_geom(selected_country, mask=True)
                  ...
                  if os.path.exists(mask_file):
                      ...
                      transposed_mask = mask.transpose(*submodel_dbuild.dims)
                      transposed_mask=transposed_mask.chunk(submodel_dbuild.chunks)
                      transposed_mask = transposed_mask.assign_coords(y=submodel_dbuild.y, x=submodel_dbuild.x)#, band=submodel_dbuild.band) #apparently they are not exactly the same
              
                      submodel_dbuild = submodel_dbuild.where(transposed_mask != 0, np.nan)
                  ...
                  submodel_dbuild = submodel_dbuild.where(fabdem != 0, np.nan)
              
                  total_damages = submodel_dbuild.sum(dim=('y', 'x'))#, 'band'))  
                  #print(' total_damages:', total_damages.compute())
                  #sys.exit()
