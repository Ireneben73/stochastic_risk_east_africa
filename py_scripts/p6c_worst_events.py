...
for submodel_id in sfincs_submodels.FID.unique().tolist():
    ...
    submodel_dbuild=xr.open_mfdataset(r'/projects/0/einf2224/paper2/scripts/model_runs/impact/submodel_' + str(submodel_id) + '_damage_build_*.nc', drop_variables='depth')
    ...
    submodel_dbuild = drop_events(submodel_dbuild) 
    
    if not submodel_dbuild.data_vars:
        ...
        # convert to integers
        submodel_dbuild = convert_to_integers(submodel_dbuild)
        print('submodel_dbuild:', submodel_dbuild)
        submodel_dbuild=submodel_dbuild.squeeze(drop=True)
        print('submodel_dbuild:', submodel_dbuild)
...
df_sum.to_csv('/projects/0/einf2224/paper2/scripts/py_scripts/output_postprocess/p6c_event_damages.csv', index=False)
